...
| Same data? | Same analysis? | Name |
|---|---|---|
| yes | yes | reproducibility check |
| yes | no | robustness / sensitivity check |
| no | yes | direct or close replication |
| no | no | conceptual replication |
...
The main message was sobering:

::: {.callout-important appearance="simple"}
# Reproducibility is not automatic
Even published, peer-reviewed findings may not reproduce.
:::

This does not mean that a field is useless.

It means that scientific claims need transparent methods, accessible materials and a traceable analysis route.
...
## Case: computational reproducibility

Sometimes the issue is not whether a theory is true.

Sometimes the issue is simpler:

<center>Can we reproduce the number in the paper?</center>

Studies of published articles with open data have found that computational reproducibility often fails because:
...
- key decisions are undocumented

::: footer
Hardwicke et al. (2018). Data availability, reusability, and analytic reproducibility. *Cognition*. Chang & Li (2015). Is economics research replicable? Federal Reserve Board.
:::

## Case: spreadsheet errors

A famous economics debate on debt and growth was influenced by spreadsheet and weighting choices.

The broader lesson is not "Excel is bad".

The lesson is:

::: {.callout-note appearance="simple"}
# Manual workflows need audit trails
If calculations are not transparent, small errors can become large arguments.
:::

In public-sector data work, a spreadsheet may influence policy. That makes documentation and checking part of professional responsibility.

::: footer
Reinhart & Rogoff (2010); Herndon, Ash, & Pollin (2013). See also public discussion of spreadsheet errors in the debt-growth debate.
:::

## Case: errors keep circulating

Wrong results do not always disappear after correction.

Retracted or corrected papers can continue to be cited, reused and believed.

::: {.callout-warning}
# Knowledge has inertia
Once an error enters the literature, it may keep moving.
:::

A reproducible archive helps reduce that risk because it makes checking easier before results become institutional knowledge.

::: footer
Anne Scheel's slides discuss retracted papers continuing to be cited, drawing on work such as Bar-Ilan & Halevi (2018).
:::
...
::: footer
Adapted from Anne Scheel's error prevention slides.
:::
...
- use Git for scripts and documents
- commit after meaningful changes
- write informative commit messages
- do not store sensitive raw data on public GitHub

::: {.callout-tip}
# In Short
Version control is a memory system for your project.
:::
...
::: footer
Motto of the Royal Society; also discussed in Anne Scheel's data sharing slides.
:::
...
For government, NGO and policy data, reproducibility requires care.

Often we cannot share:
...
- blur authorship and accountability

::: {.callout-warning}
# You remain responsible
AI assistance does not transfer scientific responsibility.
:::

## AI-use statement

In this course, document how AI influenced your project.

A useful AI-use statement contains:
...
## Case: administrative finance project

Suppose your project studies government revenue over time.

A useful archive may include:

```text
finance_project/
├── README.md
├── data-access-note.md
├── data_raw/ministry_tables_original/
├── data_clean/revenue_by_year.csv
├── code/01_import_excel.R
├── code/02_standardise_categories.R
├── code/03_trend_tables.R
├── output/figures/revenue_trend.png
├── codebook/category_definitions.md
└── decision-log.md
```

Important decisions may include category changes, inflation adjustment, missing years and revised official figures.
...
::: {.callout-tip}
# Goal
By the end of the session, your project should be easier to inspect than it was at the start.
:::
