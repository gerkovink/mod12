---
title: "C3 - Effects of the scale PeerNorms adjusted for the base set"
author: "Gerko Vink"
date: ""
output: 
   html_document:
...
fit <- d50 %>% 
  map(lm, formula = RRP_ave ~ PeerNorms + Field + Rank + Gender + Research + Support) 
...
# Multiply imputed repeated analysis 
fit <- d50 %>% 
  map(lm, formula = QRP_ave ~ PeerNorms + Field + Rank + Gender + Research + Support) 
...
```{r}
d50 %>% 
  map(select, RRfabr, RRfals, SciNorms, Field, Rank, Gender, Research, Support) %>% 
  map(RRpropodds) %>% 
  reduce(rbind) %>% 
  group_by(pars) %>% 
