---
title: "C10 - Effects of the scale Mentoring adjusted for the base set"
author: "Gerko Vink"
date: ""
output: 
   html_document:
...
fit <- d50 %>% 
  map(lm, formula = RRP_ave ~ Mentoring + Field + Rank + Gender + Research + Support)
...
# Multiply imputed repeated analysis 
fit <- d50 %>% 
  map(lm, formula = QRP_ave ~ Mentoring + Field + Rank + Gender + Research + Support)
...
```{r}
d50 %>% 
  map(select, RRfabr, RRfals, Mentoring, Field, Rank, Gender, Research, Support) %>% 
  map(RRpropodds) %>% 
  reduce(rbind) %>% 
  group_by(pars) %>% 
