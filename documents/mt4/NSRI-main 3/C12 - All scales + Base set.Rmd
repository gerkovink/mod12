---
title: "C12 - Effects of all scales adjusted for the base set"
author: "Gerko Vink"
date: ""
output: 
   html_document:
...
fit <- d50 %>% 
  map(lm, formula = RRP_ave ~ SciNorms + PeerNorms + WorkPress + PubPress + FundPress + Competitive + DetectCol + DetectRev + Mentoring + OrgJust + Field + Rank + Gender + Research + Support)
...
# Multiply imputed repeated analysis 
fit <- d50 %>% 
  map(lm, formula = QRP_ave ~ SciNorms + PeerNorms + WorkPress + PubPress + FundPress + Competitive + DetectCol + DetectRev + Mentoring + OrgJust + Field + Rank + Gender + Research + Support) 
...
fit %>% pool %>% pool.r.squared()
fit %>% pool %>% pool.r.squared(adjusted = TRUE)
...
fit <- d50 %>% 
  map(glm, 
      formula = QRP_1 ~ SciNorms + PeerNorms + WorkPress + PubPress + FundPress + Competitive + DetectCol + DetectRev + Mentoring + OrgJust + Field + Rank + Gender + Research + Support, 
      family = binomial(link = "logit"))  
...
```{r}
d50 %>% 
  map(select, RRfabr, RRfals, SciNorms, PeerNorms, WorkPress, PubPress, FundPress, Competitive, DetectCol, DetectRev, Mentoring, OrgJust, Field, Rank, Gender, Research, Support) %>% 
  map(RRpropodds) %>% 
  reduce(rbind) %>% 
  group_by(pars) %>% 
