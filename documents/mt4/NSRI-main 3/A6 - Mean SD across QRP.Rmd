---
title: "A6 Means and SD's across QRP"
author: "Gerko Vink"
date: ""
output: 
   html_document:
...
mice::complete(imp, 1) %>% #QRP has no missings -so use 1st imputed set
  select(all_of(QRP_set)) %>% 
  na_if(0) %>% 
  rowMeans(na.rm = FALSE) %>% #i.e. listwise deletion of incomplete rows
  describe() %>% .[, 2:5] %>% 
  print.data.frame()
```
...
Third, we consider the means and SD's for the QRP's for the data without the rows that contain `Not Applicable`'s where for each row the mean is calculated based on the observed (i.e. applicable) values. 
```{r}
mice::complete(imp, 1) %>% #QRP has no missings -so use 1st imputed set
  select(all_of(QRP_set)) %>% 
  na_if(0) %>% 
...
mice::complete(imp, 1) %>% #QRP has no missings -so use 1st imputed set
  ...
  mutate(rowM = rowMeans(select(., all_of(QRP_set)), na.rm = FALSE)) %>% 
  select(D2, rowM) %>% 
  melt(id.vars = "D2", value.name = "QRP_mean") %>% 
  group_by(D2) %>% 
  summarise(count = n(), 
            mean = mean(QRP_mean, na.rm=TRUE), 
            sd = sd(QRP_mean, na.rm = TRUE))  %T>%    
  print.data.frame() %>% 
...
Third, we consider the means and SD's for the QRP's where only missing cases are excluded from calculating the rowwise mean. 
```{r}
mice::complete(imp, 1) %>% #QRP has no missings -so use 1st imputed set
  select(D2, all_of(QRP_set)) %>%
  na_if(0) %>% 
  mutate(rowM = rowMeans(select(., all_of(QRP_set)), na.rm = TRUE)) %>% 
  select(D2, rowM) %>% 
  melt(id.vars = "D2", value.name = "QRP_mean") %>% 
  group_by(D2) %>% 
  summarise(count = n(), 
...
mice::complete(imp, 1) %>% #QRP has no missings -so use 1st imputed set
  ...
  mutate(rowM = rowMeans(select(., all_of(QRP_set)), na.rm = FALSE)) %>% 
  select(D3, rowM) %>% 
  melt(id.vars = "D3", value.name = "QRP_mean") %>% 
  group_by(D3) %>% 
  summarise(count = n(), 
            mean = mean(QRP_mean, na.rm=TRUE), 
            sd = sd(QRP_mean, na.rm = TRUE))  %T>%    
...
mice::complete(imp, 1) %>% #QRP has no missings -so use 1st imputed set
  ...
  mutate(rowM = rowMeans(select(., all_of(QRP_set)), na.rm = FALSE)) %>% 
  select(D4, rowM) %>% 
  melt(id.vars = "D4", value.name = "QRP_mean") %>% 
  group_by(D4) %>% 
  summarise(count = n(), 
            mean = mean(QRP_mean, na.rm=TRUE), 
            sd = sd(QRP_mean, na.rm = TRUE))  %T>%    
...
mice::complete(imp, 1) %>% #QRP has no missings -so use 1st imputed set
  ...
  mutate(rowM = rowMeans(select(., all_of(QRP_set)), na.rm = FALSE)) %>% 
  select(D5, rowM) %>% 
  melt(id.vars = "D5", value.name = "QRP_mean") %>% 
  group_by(D5) %>% 
  summarise(count = n(), 
            mean = mean(QRP_mean, na.rm=TRUE), 
            sd = sd(QRP_mean, na.rm = TRUE))  %T>%    
...
mice::complete(imp, 1) %>% #QRP has no missings -so use 1st imputed set
  ...
  mutate(rowM = rowMeans(select(., all_of(QRP_set)), na.rm = FALSE)) %>% 
  select(Group, rowM) %>% 
  melt(id.vars = "Group", value.name = "QRP_mean") %>% 
  group_by(Group) %>% 
  summarise(count = n(), 
            mean = mean(QRP_mean, na.rm=TRUE), 
            sd = sd(QRP_mean, na.rm = TRUE))  %T>%    
