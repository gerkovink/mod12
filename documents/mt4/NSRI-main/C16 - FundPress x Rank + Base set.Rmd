---
title: "C16 - Effects of FundPress x Rank adjusted for the base set"
author: "Gerko Vink"
date: ""
output: 
   html_document:
...
fit <- d50 %>% 
  map(lm, formula = RRP_ave ~ FundPress * Rank + Field + Gender + Research + Support)
...
# Multiply imputed repeated analysis 
fit <- d50 %>% 
  map(lm, formula = QRP_ave ~ FundPress * Rank + Field + Gender + Research + Support)
...
fit <- d50 %>% 
  map(glm, 
      formula = QRP_1 ~ FundPress * Rank + Field + Gender + Research + Support, 
      family = binomial(link = "logit"))  
...
```{r eval = FALSE}
d50 %>% 
  map(select, RRfabr, RRfals, FundPress, Field, Rank, Gender, Research, Support) %>%
  map(~ .x %>% 
        mutate(CompxField = FundPress * Rank)) %>% 
  map(RRpropodds) %>% 
  reduce(rbind) %>% 
  group_by(pars) %>% 
