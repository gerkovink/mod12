---
title: "C13 - Effects of Field x Rank adjusted for the base set"
author: "Gerko Vink"
date: ""
output: 
   html_document:
...
fit <- d50 %>% 
  map(glm, 
      formula = QRP_1 ~ Field * Rank + Gender + Research + Support, 
      family = binomial(link = "logit"))  
...
```{r eval=FALSE}
d50 %>% 
  map(select, RRfabr, RRfals, Field, Rank, Gender, Research, Support) %>%
  map(~ .x %>% 
        mutate(FieldxRank = Field * Rank)) %>% 
  map(RRpropodds) %>% 
  reduce(rbind) %>% 
  group_by(pars) %>% 
