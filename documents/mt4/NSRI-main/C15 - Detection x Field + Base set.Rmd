---
title: "C15 - Effects of Detection x Field adjusted for the base set"
author: "Gerko Vink"
date: ""
output: 
   html_document:
...
fit <- d50 %>% 
  map(lm, formula = RRP_ave ~ DetectRev * DetectCol * Field + Rank + Gender + Research + Support) 
...
# Multiply imputed repeated analysis 
fit <- d50 %>% 
  map(lm, formula = QRP_ave ~ DetectRev * DetectCol * Field + Rank + Gender + Research + Support) 
...
fit <- d50 %>% 
  map(glm, 
      formula = QRP_1 ~ Field * DetectRev + Gender + Research + Support, 
      family = binomial(link = "logit"))  
...
```{r eval=FALSE}
d50 %>% 
  map(select, RRfabr, RRfals, DetectRev, Field, Rank, Gender, Research, Support) %>%
  map(~ .x %>% 
        mutate(DetectxField = Field * DetectRev)) %>% 
  map(RRpropodds) %>% 
  reduce(rbind) %>% 
  group_by(pars) %>% 
