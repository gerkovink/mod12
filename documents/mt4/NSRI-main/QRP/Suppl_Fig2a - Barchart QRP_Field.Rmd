...
d <- d50[[1]] %>% #RRP has no missings -so use 1st imputed set
  select(Field, starts_with("Q")) %>% select(1:12)
names(d) <- c("Field", paste("QRP", 1:11, sep = ""))

a <- d %>% gather(key, value, -Field, factor_key = TRUE) %>% 
  count(Field, key, value) %>% 
  add_count(Field, key, wt = n, name = "sum") %>% 
...
pdf(file = "Suppl_Fig2a_Field.pdf", width = 10, height = 6)
a
dev.off()

postscript(file = "Suppl_Fig2a_Field_large.eps", width = 10, height = 6)
a
dev.off()
```
