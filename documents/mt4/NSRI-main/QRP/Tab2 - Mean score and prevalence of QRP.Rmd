...
RR2 <- function(df){
  ...
  data.frame(df, phat = phat, lwr = min95, upr = max95, p.ci = paste(round(phat, 1), paste("(", round(min95, 1), ",", round(max95, 1), ")", sep = "")))
}
...
```{r echo = FALSE}
Overall <- d50[[1]] %>% 
  select(contains("Q"), QRP_ave) %>% # select relevant columns
  select(!contains("_"), QRP_ave) %>% # exclude raw
...
```{r echo = FALSE}
Field <- d50[[1]] %>% 
  select(contains("Q"), QRP_ave,  Field) %>% # select relevant columns
  select(!contains("_"), QRP_ave, Field) %>% # exclude raw
...
```{r echo = FALSE}
Rank <- d50[[1]] %>% 
  select(contains("Q"), QRP_ave, Rank) %>% # select relevant columns
  select(!contains("_"), QRP_ave, Rank) %>% # exclude raw
...
FF_Rank <- d50[[1]] %>% 
  ...
  group_map(~.x %$% 
              ...
              melt(value.name = "Freq")) %>% 
              map(~.x %>% RR2() %>%  .$phat %>% .[2:4] %>% sum) %>% 
   do.call(rbind, .) %>% t() %>% as.data.frame() %>% round(1) %>% 
  mutate(Indicator = "FF") 
