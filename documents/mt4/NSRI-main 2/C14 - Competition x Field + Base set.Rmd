---
title: "C14 - Effects of Competitive x Field adjusted for the base set"
author: "Gerko Vink"
date: ""
output: 
   html_document:
...
fit <- d50 %>% 
  map(lm, formula = RRP_ave ~ Competitive * Field + Rank + Gender + Research + Support)
...
# Multiply imputed repeated analysis 
fit <- d50 %>% 
  map(lm, formula = QRP_ave ~ Competitive * Field + Rank + Gender + Research + Support)
...
fit <- d50 %>% 
  map(glm, 
      formula = QRP_1 ~ Competitive * Field + Rank + Gender + Research + Support, 
      family = binomial(link = "logit"))  
...
```{r eval=FALSE}
d50 %>% 
  map(select, RRfabr, RRfals, Competitive, Field, Rank, Gender, Research, Support) %>%
  map(~ .x %>% 
        mutate(CompxField = Competitive * Field)) %>% 
  map(RRpropodds) %>% 
  reduce(rbind) %>% 
  group_by(pars) %>% 
