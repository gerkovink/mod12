...
d <- d50[[1]] %>% #RRP has no missings -so use 1st imputed set
  select(Support, starts_with("Q")) %>% select(1:12)
names(d) <- c("Support", paste("QRP", 1:11, sep = ""))

a <- d %>% gather(key, value, -Support, factor_key = TRUE) %>% 
  count(Support, key, value) %>% 
  add_count(Support, key, wt = n, name = "sum") %>% 
...
pdf(file = "Suppl_Fig2e_Support.pdf", width = 10, height = 5)
a
dev.off()

postscript(file = "Suppl_Fig2e_Support_large.eps", width = 10, height = 5)
a
dev.off()
```
